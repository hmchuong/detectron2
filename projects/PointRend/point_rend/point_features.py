...
def get_error_coords_with_randomness(
    error_score, num_points, oversample_ratio, importance_sample_ratio
):
    """
    ...
    Args:
        error_score (Tensor): A tensor of shape (N, C, Hmask, Wmask) or (N, 1, Hmask, Wmask) for
    ...
    num_boxes = error_score.shape[0]
    num_sampled = int(num_points * oversample_ratio)
    point_coords = torch.rand(num_boxes, num_sampled, 2, device=error_score.device)
    point_uncertainties = point_sample(error_score, point_coords, align_corners=False)

    num_uncertain_points = int(importance_sample_ratio * num_points)
    num_random_points = num_points - num_uncertain_points
    idx = torch.topk(point_uncertainties[:, 0, :], k=num_uncertain_points, dim=1)[1]
    shift = num_sampled * torch.arange(num_boxes, dtype=torch.long, device=error_score.device)
    idx += shift[:, None]
    ...
    if num_random_points > 0:
        point_coords = cat(
            [
                point_coords,
                torch.rand(num_boxes, num_random_points, 2, device=error_score.device),
            ],
            dim=1,
        )
    return point_coords
